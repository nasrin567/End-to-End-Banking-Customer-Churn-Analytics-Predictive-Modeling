...
df_train.Balance / df_train.Salary
...
categorical_values = {
    col: df_train[col].unique().tolist()
    for col in cat_cols
}

for col in cat_cols:
    for val in df_train[col].unique():
        df_train[f"{col}_{val}"] = np.where(
            df_train[col] == val, 1, 0
        )
...
# %%
# =============================================
# Save Preprocessing Bundle for Deployment
# =============================================
...
preprocessing_bundle = {
    ...
    "num_cols": num_cols,
    "cat_cols": cat_cols,
    "feature_columns": df_train_x.columns.tolist(),
    "categorical_values": categorical_values
}

joblib.dump(
    preprocessing_bundle,
    "predictive_modelling/processed_data/preprocessing_bundle.pkl"
)

print("Preprocessing bundle saved successfully")

# %%
import joblib

data = joblib.load("dataset_bundle.pkl")
